...
@property (nonatomic, strong) UIImageView *qu_CIcon;
@property (nonatomic, strong) UIImageView *huan_CIcon;
...
//    _CTimeLbl.text = @"7天";
    ...
    _qu_CIcon = [UIImageView new];
    _qu_CIcon.hidden = YES;
    _qu_CIcon.image = JGImage(@"icon_qu");

    _huan_CIcon = [UIImageView new];
    _huan_CIcon.hidden = YES;
    _huan_CIcon.image = JGImage(@"icon_huan");
    ...
    [_TopView addSubview:_qu_CIcon];
    [_TopView addSubview:_huan_CIcon];
    ...
    [_qu_CIcon mas_updateConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_CTimeLbl);
        make.right.equalTo(_CTimeLbl.mas_left);
        make.size.equalTo(@(CGSizeMake(12, 6)));
    }];
    [_huan_CIcon mas_updateConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_CTimeLbl);
        make.left.equalTo(_CTimeLbl.mas_right);
        make.size.equalTo(@(CGSizeMake(12, 6)));
    }];
...
- (void)CalculateDate {
    ...
    if (self.LeftModel.toString.length && self.LeftModel.timeStr.length) {
        
        _LDateLbl.text = [NSString stringWithFormat:@"%ld月%ld日",self.LeftModel.month, self.LeftModel.day];
        _LTimeLbl.text = [self.LeftModel.getWeek stringByAppendingString:self.LeftModel.timeStr];
    }else {
    ...
    if (self.RightModel.toString.length) {
        ...
        _RDateLbl.text = [NSString stringWithFormat:@"%ld月%ld日",self.RightModel.month, self.RightModel.day];
        _RTimeLbl.text = [self.RightModel.getWeek stringByAppendingString:self.RightModel.timeStr];
    }else {
    ...
    _qu_CIcon.hidden = _CTimeLbl.hidden;
    _huan_CIcon.hidden = _CTimeLbl.hidden;
...
-(void)fy_settingBtnSelectedStatesWithType:(NSString *)type
{
