...
- (BOOL)isItFutureday
{
    ...
    NSDate *selfDate = [fmt dateFromString:selfString];
    NSDate *nowDate = [self fy_getthisTimeString:nowString nextYear:0 nextMoth:0 nextDay:365];
    
    [fmt dateFromString:nowString];
    
    NSComparisonResult result = [nowDate compare:selfDate];
...
//    JGLog(@"%ld",delta.minute);
    ...
    if (house != 0 || minute != 0) {
        return [NSString stringWithFormat:@"共%ld天",day + 1];
    }else {
        return [NSString stringWithFormat:@"共%ld天",day];
    }
}
...
-(NSDate *)fy_getthisTimeString:(NSString *)timeString nextYear:(NSInteger)nextYear nextMoth:(NSInteger)nextMoth nextDay:(NSInteger)nextDay
{
    ...
    [formatter setDateFormat:@"yyyy-MM-dd"]; //这里的格式必须和String格式一致
